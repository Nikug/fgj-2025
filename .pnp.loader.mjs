               ...
               if (opts.withFileTypes) {
                  ...
                  this.realFs.readdir(
                     ...
                     this.makeCallback(
                        results =>
                           resolve(
                              results.map(npath.toPortablePath),
                           ),
                        reject,
                     ),
                  );
...
const URL =
   Number(process.versions.node.split('.', 1)[0]) < 20 ? URL$1 : (
      globalThis.URL
   );
...
async function load$1(urlString, context, nextLoad) {
   ...
   if (process.env.WATCH_REPORT_DEPENDENCIES && process.send) {
      ...
      process.send({
         'watch:import':
            WATCH_MODE_MESSAGE_USES_ARRAYS ?
               [pathToSend]
            :  pathToSend,
      });
   }
...
function getPackageConfig(path, specifier, base, readFileSyncFn) {
   ...
   const exports =
      ObjectPrototypeHasOwnProperty(packageJSON, 'exports') ?
         packageJSON.exports
      :  void 0;
